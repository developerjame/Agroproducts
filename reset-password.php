...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($new_password_err) && empty($confirm_password_err)){
        ...
        $sql = "UPDATE farmer SET fpassword = ? WHERE fid = ?";
        
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "si", $param_fpassword, $param_fid);
            
            // Set parameters
            $param_fpassword = password_hash($new_password, PASSWORD_DEFAULT);
            $param_fid = $_SESSION["fid"];
    ...
    <div class="contact-form">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post"> 
            ...
            <div class="form-group">
                ...
                <a style="color: blue;font-size: 20px;" class="btn btn-link" href="profileView.php">Cancel</a>
            </div>
        </form>
    </div>    
</body>
